// =================================================================================================
// Flash Attention f32/q4_0 - Optimized Version (Production)
// =================================================================================================
// f16 커널과 동일한 최적화 기법 적용 (q4_0 dequantization 로직 유지):
//   - 1-Pass Online Softmax (decode & prefill)
//   - Subgroup Reduce (sub_group_reduce_add)
//   - Q pre-scale (register-resident)
//   - KV_UNROLL=4 배치 처리
//   - Conditional rescale + native_exp
//   - Barrier-free single-Q prefill
//   - K-only local memory for old prefill (V global direct access)
// =================================================================================================
...
// =================================================================================================
// q4_0 dequantization helpers
// =================================================================================================

// Dequantize 2 consecutive elements at position 'dim' (must be even-aligned)
inline float2 dequant_q4_0_2(const global struct block_q4_0 * row_blocks, int dim) {
    const int bi  = dim / QK4_0;
    const int off = dim % QK4_0;
    const global struct block_q4_0 * b = &row_blocks[bi];
    const float d = (float)b->d;

    if (off < 16) {
        return (float2)(
            ((int)(b->qs[off]     & 0x0F) - 8) * d,
            ((int)(b->qs[off + 1] & 0x0F) - 8) * d
        );
    } else {
        const int qi = off - 16;
        return (float2)(
...
inline float4 dequant_q4_0_4(const global struct block_q4_0 * row_blocks, int col) {
    const int bi  = col / 8;
    const int vi  = col % 8;
    const global struct block_q4_0 * b = &row_blocks[bi];
    const float d = (float)b->d;
...
// =================================================================================================
__kernel void flash_attn_f32_q4_0_old(
    ...
    // K-only local memory (V는 global direct access → LDS 50% 절감)
    ...
    for (int k_start = 0; k_start < n_kv; k_start += BLOCK_N) {
        ...
        // K 타일 → local memory (q4_0 dequant → half4)
        #pragma unroll 1
        ...
        if (valid_query) {
            ...
            for (int j = 0; j < k_tile_size; j += UNROLL_FACTOR) {
                ...
                // V accumulation - direct global access with on-the-fly q4_0 dequant
                #pragma unroll
                for (int i = 0; i < DV_VEC; ++i) {
                    ...
                    for (int w = 0; w < UNROLL_FACTOR; ++w) {
                        ...
                        if (k_row < n_kv && p[w] > 0.0f) {
                            const global struct block_q4_0 * v_blocks =
                                (const global struct block_q4_0 *)(v_base + v_base_offset + k_row * v_nb1);
                            float4 v_val = dequant_q4_0_4(v_blocks, i);
...
//
// f16 decode 커널과 동일한 최적화 패턴:
//   - K/V base 포인터 사전 계산
//   - Q에 scale 사전 곱셈
//   - DK=128 경로: bounds-check 제거
//   - DEC_KV_UNROLL=4 배치: rescale 횟수 75% 감소, exp 37.5% 감소
//   - Conditional rescale: max가 변할 때만 exp(alpha) 계산
//   - native_exp 사용
//   - q4_0 dequantization constants 사전 계산
// =================================================================================================
...
__kernel void flash_attn_f32_q4_0_q1(
    ...
    // ===== Dequant constants (loop-invariant) =====
    const int k_bi  = d / QK4_0;
    const int k_off = d % QK4_0;
    const int k_hi  = (k_off >= 16);
    const int k_qi  = k_hi ? (k_off - 16) : k_off;
    const int k_shift = k_hi ? 4 : 0;

    const int v_bi  = d / QK4_0;
    const int v_off = d % QK4_0;
    const int v_hi  = (v_off >= 16);
    const int v_qi  = v_hi ? (v_off - 16) : v_off;
...
#if DK == 128
            const global struct block_q4_0 * kb =
                (const global struct block_q4_0 *)(k_ptr_base + (ulong)(ki + w) * k_nb1) + k_bi;
...
#if DV == 128
            const global struct block_q4_0 * vb =
                (const global struct block_q4_0 *)(v_ptr_base + (ulong)(ki + w) * v_nb1) + v_bi;
...
#if DK == 128
        const global struct block_q4_0 * kb =
            (const global struct block_q4_0 *)(k_ptr_base + (ulong)ki * k_nb1) + k_bi;
        const float kd = (float)kb->d;
        const float2 kv = (float2)(
            ((int)((kb->qs[k_qi]     >> k_shift) & 0x0F) - 8) * kd,
            ((int)((kb->qs[k_qi + 1] >> k_shift) & 0x0F) - 8) * kd
...
#if DV == 128
        const global struct block_q4_0 * vb =
            (const global struct block_q4_0 *)(v_ptr_base + (ulong)ki * v_nb1) + v_bi;
        const float vd = (float)vb->d;
        const float2 vv = (float2)(
            ((int)((vb->qs[v_qi]     >> v_shift) & 0x0F) - 8) * vd,
            ((int)((vb->qs[v_qi + 1] >> v_shift) & 0x0F) - 8) * vd
...
__kernel void flash_attn_f32_q4_0(
    ...
    const int k_bi  = d / QK4_0;
    const int k_off = d % QK4_0;
    const int k_shift = (k_off >= 16) ? 4 : 0;
    const int k_qi  = (k_off >= 16) ? (k_off - 16) : k_off;

    const int v_bi  = d / QK4_0;
    const int v_off = d % QK4_0;
    const int v_shift = (v_off >= 16) ? 4 : 0;
    const int v_qi  = (v_off >= 16) ? (v_off - 16) : v_off;
