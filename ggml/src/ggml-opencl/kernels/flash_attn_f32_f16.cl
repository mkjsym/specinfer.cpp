...
// =================================================================================================
// 2. Decoding Kernel — GQA-Grouped Dispatch (Adreno 750, wave=64)
//
//   - GQA-grouped WG: min(gqa_ratio, 4) Q heads per WG, sharing KV head
//     → forces L1 cache co-location for K and V data
//   - K/V base pointers pre-computed per KV head
//   - Q pre-scaled, DK=128 branch elimination, conditional rescale
//   - DEC_KV_UNROLL=4 batch: 75% fewer rescales
// =================================================================================================
#ifdef ADRENO_GPU
...
__kernel void flash_attn_f32_f16_q1(
    ...
    // GQA-grouped dispatch: Q heads sharing a KV head are co-located in the
    // same workgroup to guarantee L1 cache sharing for K and V data.
    // Host launches: local={64*heads_per_wg, 1}, global={same, n_head_kv*sg_groups*n_batch}
    const int sg_lid = get_sub_group_local_id();
    const int sg_id  = get_sub_group_id();
    const int n_sg   = get_num_sub_groups();

    const int gqa_ratio = n_head / n_head_kv;
    const int sg_groups = (gqa_ratio + n_sg - 1) / n_sg;
    const int wg1  = get_group_id(1);
    const int bi   = wg1 / (n_head_kv * sg_groups);
    const int rest = wg1 % (n_head_kv * sg_groups);
    const int hkv  = rest / sg_groups;
    const int hi   = hkv * gqa_ratio + (rest % sg_groups) * n_sg + sg_id;
    if (hi >= n_head) return;

    const int tid = sg_lid;
    const int d = tid * 2;
    ...
    for (; ki < kv_batched; ki += DEC_KV_UNROLL) {
...
#ifdef ADRENO_GPU
        prefetch(vr_base + ki * v_stride_h + (tid >> 1) * 4, 4);
#endif
        ...
        if (ki + DEC_KV_UNROLL < kv_batched)
            prefetch(vr_base + (ki + DEC_KV_UNROLL) * v_stride_h + (tid >> 1) * 4, 4);
#endif
    }
...
// =================================================================================================
// 3. Prefill Kernel — GQA-Grouped, Barrier-Free (Adreno 750, wave=64)
//
//   - GQA-grouped WG: min(gqa_ratio, 4) Q heads per WG, sharing KV head
//     → forces L1 cache co-location for K and V data
//   - Each subgroup(64 lanes) = 1 (batch, head, query_row)
//   - Barrier 0회 (subgroup intrinsic만 사용)
//   - LDS 0 bytes (Q register, K/V global → L1/L2 cache)
//   - K: read_imagef(float precision) → texture L1, fp32 QK dot
//   - V: coalesced vload_half4 on Adreno (2 lanes share 1 load)
//   - Q pre-scaled, DK=128 branch elimination, conditional rescale
// =================================================================================================
#define KV_UNROLL 4
...
__kernel void flash_attn_f32_f16(
    ...
    // GQA-grouped dispatch: Q heads sharing a KV head are co-located in the
    // same workgroup to guarantee L1 cache sharing for K and V data.
    // Host launches: local={64*heads_per_wg, 1}, global={same, n_head_kv*sg_groups*n_batch*n_q}
    const int sg_lid = get_sub_group_local_id();
    const int sg_id  = get_sub_group_id();
    const int n_sg   = get_num_sub_groups();
    ...
    const int wgs_per_batch = n_head_kv * sg_groups * n_q;
    const int bi   = wg1 / wgs_per_batch;
    const int rem  = wg1 % wgs_per_batch;
    const int hkv  = rem / (sg_groups * n_q);
    const int rem2 = rem % (sg_groups * n_q);
    const int hi   = hkv * gqa_ratio + (rem2 / n_q) * n_sg + sg_id;
    const int qr   = rem2 % n_q;

    if (hi >= n_head || qr >= n_q) return;
    ...
    // ===== Main loop: KV_UNROLL KV tokens per iteration =====
    ...
    for (; ki < kv_batched; ki += KV_UNROLL) {
...
#ifdef ADRENO_GPU
        // V prefetch: issue per-lane targeted prefetch during K texture read latency
        prefetch(vr_base + ki * v_stride_h + (tid >> 1) * 4, 4);
#endif

        // Phase 1: K reads + QK dot products (texture path on Adreno, buffer on others)
        #pragma unroll
        ...
        // Phase 3: V loads + accumulation (coalesced half4 on Adreno, half2 on others)
        #pragma unroll
        for (int w = 0; w < KV_UNROLL; ++w) {
            const float p = native_exp(s[w] - mi);
            li += p;
...
#ifdef ADRENO_GPU
        // Prefetch V for next batch (per-lane targeted)
        if (ki + KV_UNROLL < kv_batched)
            prefetch(vr_base + (ki + KV_UNROLL) * v_stride_h + (tid >> 1) * 4, 4);
#endif
    }
