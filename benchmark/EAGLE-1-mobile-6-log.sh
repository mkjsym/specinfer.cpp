...
EXECUTABLE="./llama-speculative-eagle-6-log"
...
# 추론 파라미터 (요청된 사항으로 수정)
INFERENCE_ARGS="-c 0 --color --top-k 4 -fa off --top-p 1.0 --min-p 0.0 --temp 0.0 --draft-max 7 --draft-min 1 --n-predict 200 -ngl 40 -ngld 20 -np 2 -s 1234 -kvu"

# 로그 파일 이름 설정 (adb shell의 'date' 명령어 형식에 주의)
LOG_FILE="/data/local/tmp/youngmin/eagle-2/benchmark/logs/EAGLE-1-6-mobile_Q4_0_log_$(date +%Y-%m-%d_%H-%M-%S).txt"
