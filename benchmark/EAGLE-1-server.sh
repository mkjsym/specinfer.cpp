...
# --- 설정 ---
# 실행 파일 경로
EXECUTABLE="build/bin/llama-speculative-eagle"
# 모델 및 추론 파라미터
MODEL_PATH_TGT="/data/youngmin/models/eagle_models/vicuna.gguf"
MODEL_PATH_DFT="/data/youngmin/models/eagle_models/EAGLE-vicuna7b.gguf"
INFERENCE_ARGS="-c 0 --color --top-k 4 -fa off --temp 0.0 --top-p 1.0 --min-p 0.0 --draft-max 25 --draft-min 1 --n-predict 200 -ngl 40 -ngld 20 -np 20 -s 1234 -kvu"
# [추가] 로그 파일 이름 설정 (날짜와 시간 포함)
LOG_FILE="./benchmark/logs/EAGLE-1-server_$(date +%Y-%m-%d_%H-%M-%S).txt"
